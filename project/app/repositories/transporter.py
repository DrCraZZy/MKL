...
class TransporterRepository(BaseRepository):
    ...
    async def create_transporter(self, transporter: TransporterSchema) -> EndpointAnswer:
        if not await is_inn_exists(transporter.inn, transporter_data, self.database):
            ...
            message = f"There is already transporter with inn:'{transporter.inn}' in the table."
            result = EndpointAnswer(status="fail", message=message)
            logger.info(message)
        return result
    ...
    async def update_transporter_by_inn(self, transporter_inn: str, transporter: TransporterSchema) -> EndpointAnswer:
        if await is_inn_exists(transporter_inn, transporter_data, self.database):
            ...
            message = f"There is no transporter with inn:'{transporter_inn}' for update in the table."
            result = EndpointAnswer(status="fail", message=message)
            logger.info(message)
    ...
    async def delete_transporter_by_inn(self, transporter_inn: str) -> EndpointAnswer:
        if await is_inn_exists(transporter_inn, transporter_data, self.database):
            ...
            message = f"There is no transporter with inn:'{transporter_inn}' in the table."
            result = EndpointAnswer(status="fail", message=message)
            logger.info(message)
        return result
